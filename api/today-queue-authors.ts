...
type TodayQueueAuthorsResponse = {
  status: (statusCode: number) => TodayQueueAuthorsResponse
  setHeader: (key: string, value: string) => TodayQueueAuthorsResponse
...
function getBearerToken(request: TodayQueueAuthorsRequest) {
...
function sendJson(response: TodayQueueAuthorsResponse, statusCode: number, payload: unknown) {
  response.status(statusCode).setHeader('content-type', 'application/json')
  response.setHeader('cache-control', 'no-store')
  response.end(JSON.stringify(payload))
}

async function readBody(request: TodayQueueAuthorsRequest) {
...
export default async function handler(
  request: TodayQueueAuthorsRequest,
  response: TodayQueueAuthorsResponse,
) {
  ...
  try {
    ...
    const supabaseResponse = await fetchWithTimeout(
      `${url}/rest/v1/rpc/get_today_queue_authors`,
      {
        method: 'POST',
        headers: {
          apikey: anonKey,
    ...
    if (!supabaseResponse.ok) {
      sendJson(response, supabaseResponse.status, {
        error: getSupabaseErrorMessage(responseBody, 'Today queue authors request failed.'),
      })
      return
    }
