...
declare
  ...
  has_active_gasoline_limit boolean := false;
  touches_gasoline_preference boolean := false;
begin
  ...
  touches_gasoline_preference :=
    reservation_row.fuel_type in ('AI_92', 'AI_95', 'AI_100')
    or update_reservation_fuel_preference.fuel_type in ('AI_92', 'AI_95', 'AI_100')
    or coalesce(reservation_row.fuel_preference_mode, 'EXACT') = 'ANY_GASOLINE'
    or effective_fuel_preference_mode = 'ANY_GASOLINE';
  ...
  select exists (
    ...
    join public.daily_fuel_type_limits dftl
     ...
     and dftl.fuel_category = 'GASOLINE'
    where dl.date = current_date
      and dl.station_id is null
      and (
        (dftl.limit_mode = 'vehicle_count' and coalesce(dftl.vehicle_limit, 0) > 0)
        or (dftl.limit_mode = 'fuel_liters' and coalesce(dftl.liters_limit, 0) > 0)
      )
  )
  into has_active_gasoline_limit;

  if touches_gasoline_preference
    and has_active_gasoline_limit
    and (
    ...
    raise exception 'FUEL_PREFERENCE_LOCKED_BY_ACTIVE_GASOLINE_LIMIT';
  end if;
