...
declare
  ...
  effective_fuel_category_filter text := nullif(fuel_category_filter, '');
begin
  ...
  if effective_fuel_category_filter is not null
    and effective_fuel_category_filter not in ('GASOLINE', 'DIESEL', 'GAS') then
    raise exception 'INVALID_FUEL_CATEGORY_FILTER';
  end if;
  ...
  return (
    ...
    page_base as (
      ...
      from filtered_base
      where (
          effective_fuel_category_filter is null
          or public.get_fuel_queue_category(fuel_type) = effective_fuel_category_filter
        )
    ),
    page_candidates as (
      select *
