...
drop function if exists public.create_daily_limit(date, uuid, integer, numeric, jsonb, uuid);
drop function if exists public.create_daily_limit(date, text, integer, numeric, text, integer, numeric, text, integer, numeric, uuid);
drop function if exists public.create_daily_limit(date, jsonb, uuid);
...
begin
  ...
  if target_station_id is null or not exists (
    ...
    where s.id = target_station_id and s.is_active
  ) then
  ...
  loop
    ...
    if item_vehicle_limit < 0 or (item_status = 'OPEN' and item_vehicle_limit = 0) then
